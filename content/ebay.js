...
async function fillListing(listing) {
  ...
  return {
    ok: true,
    warning: "eBay is still beta. DOM posting is scaffolded only; API fallback is not implemented."
  };
}
...
async function submitListing() {
  ...
  if (!submitButton) {
    return { ok: false, error: "eBay submit button not found." };
  }
...
async function injectPhotos(base64Array, fileInput) {
  const files = await Promise.all(
    base64Array.map(async (base64, index) => {
      ...
      return new File([blob], `ebay_photo_${index + 1}.jpg`, { type: blob.type || "image/jpeg" });
    })
  );

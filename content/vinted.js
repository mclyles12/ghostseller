...
async function fillListing(listing) {
  ...
  return {
    ok: true,
    warning: "Vinted fill flow is scaffolded but selectors and upload behavior need live validation."
  };
}
...
async function submitListing() {
  ...
  if (!submitButton) {
    return { ok: false, error: "Vinted submit button not found." };
  }
...
async function injectPhotos(base64Array, fileInput) {
  const files = await Promise.all(
    base64Array.map(async (base64, index) => {
      ...
      return new File([blob], `vinted_photo_${index + 1}.jpg`, { type: blob.type || "image/jpeg" });
    })
  );
